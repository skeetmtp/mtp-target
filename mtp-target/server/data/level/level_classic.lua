Name = "Snow classic"
Author = "Skeet and 9dan"
ServerLua = "level_classic_server.lua"

skyShapeFileName = "sky.shape"

Cameras =
{
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10),
	CVector(0, 3, 10)
}

StartPoints =
{
	CVector(-0.098865, 1.56397, 9.68978),
	CVector(-0.25109, 1.56534, 9.68978),
	CVector(-0.411029, 1.55867, 9.68978),
	CVector(-0.570633, 1.55203, 9.68978),
	CVector(0.0733468, 1.5695, 9.68978),
	CVector(0.225997, 1.56282, 9.68978),
	CVector(0.394393, 1.55616, 9.68978),
	CVector(0.554403, 1.54953, 9.68978),
	CVector(-0.178982, 1.3806, 9.68978),
	CVector(-0.331207, 1.38197, 9.68978),
	CVector(-0.491146, 1.37531, 9.68978),
	CVector(-0.65075, 1.36866, 9.68978),
	CVector(-0.00677051, 1.38614, 9.68978),
	CVector(0.14588, 1.37946, 9.68978),
	CVector(0.314275, 1.3728, 9.68978),
	CVector(0.474286, 1.36616, 9.68978)
}

Modules =
{
	{ Position = CVector(10, -25, 3.5),Rotation = CAngleAxis(1,0,0,0),			Name = "snow_island" },
	{ Position = CVector(6, -14, 2.8),Rotation = CAngleAxis(1,0,0,0),			Name = "snow_island2" },
	{ Position = CVector(-3.8, -17.0, 2),Rotation = CAngleAxis(1,0,0,0),		Name = "snow_island3" },
	{ Position = CVector(-0.1, -15.3, 1),Rotation = CAngleAxis(1,0,0,0),		Name = "snow_target_50" },
	{ Position = CVector(-0.1, -15.35, 2.55),Rotation = CAngleAxis(1,0,0,0),	Name = "snow_target_100" },
	{ Position = CVector(-0.1, -15.33, 2.8),Rotation = CAngleAxis(1,0,0,0),	Name = "snow_target_300" },
	{ Position = CVector(0, 0, 5),Rotation = CAngleAxis(1,0,0,0),				Name = "snow_ramp" }
}


