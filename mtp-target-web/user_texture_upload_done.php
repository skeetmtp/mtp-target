...
require_once ("stat_function.php");
	...
	if (isset ($user_texture_id)) 
	{
		if (validNumber($user_texture_id)) 
		{
			...
			printf("<a href=\"?page=user_settings&user_texture_id=%s\"><img src=\"%s\" ALT=\"user texture(id=%d) : %s\"></a><br/>\n",$user_texture_id,$userTextureUrl,$user_texture_id,$userTextureName);
			printf("<a href=\"?page=user_settings&user_texture_id=%s\">Click here to set this texture as your default one.</a><br/>\n",$user_texture_id,$userTextureUrl,$user_texture_id,$userTextureName);
		}
		else
