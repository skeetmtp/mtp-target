...
include_once("stat_game.php");
	...
	if($uid==0)	
		...
		$requete = "SELECT count(*),DAYOFMONTH(session.Date) as d1 ,DAYOFMONTH(session.Date) as d2 FROM session,user_session WHERE MONTH(session.Date)=$monthToStat AND YEAR(session.Date)=$yearToStat AND session.Id=user_session.SessionId AND user_session.UId=$uid GROUP BY TO_DAYS(session.Date);";
